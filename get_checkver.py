...
cql_get_updateAt = "select objectId, requestObj from StatLog where updatedAt >= date('"+date_from+"') and updatedAt <= date('"+date_to+"') limit 99999999"
...
for each in todo_list_updateAt:
    print(each.get('requestObj')['headerRequest']['action'])
    if each.get('requestObj')['headerRequest']['action'] != 'STAT_LOGIN' and each.get('requestObj')['headerRequest']['projectId'] == 'ota_002':
        i=i+1
        print(i)
